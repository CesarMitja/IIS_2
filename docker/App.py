...
model_path = os.path.join(current_directory, 'best_model_LSTM.keras')
...
def napoved_naloga6():
    ...
    if len(timeseries) < 24:
        return jsonify({'error': 'Invalid timeseries length (too short). Expected 24 data points'}), 400
    if len(timeseries) > 24:
        return jsonify({'error': 'Invalid timeseries length (too long). Expected 24 data points'}), 400

    timeseries_reshaped = np.array([timeseries]).reshape(1, 1, 24)
